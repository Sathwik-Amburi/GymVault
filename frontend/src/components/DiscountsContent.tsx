...
const DiscountsPage: React.FC<DiscountsPageProps> = ({
  ...
  const handleSubmit = async (
    ...
    let x = subscriptionOffers.map((subscriptionOffer: subscriptionOffer) => {
      if (subscriptionTypes.includes(subscriptionOffer.subscriptionType)) {
        ...
        return {
          subscriptionType: newOffers[index][0],
          subscriptionPrice: newOffers[index][1],
          discount: newOffers[index][2],
        };
      } else {
  ...
  return (
    <>
      ...
      <Button
        onClick={(event: any) => handleSubmit(event, subscriptionOffers)}
        style={{ float: "right" }}
        variant="contained"
      >
        Save
